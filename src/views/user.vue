<template>
  <div>
    ...
    <div class="xx">
      <div class="tx">
        <input type="file" class="info" @change="urlimg" ref="file" />
        <h2>头像</h2>
        <div class="tu">
          <img :src="user.avatar" />
          <span>></span>
      ...
      <div class="btn" @click="openfalse">退出登录</div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    userlist() {
      this.axios
        .get(`http://elm.cangdu.org/v1/user?user_id=${localStorage.userid}`)
        .then(res => {
          console.log(res.data);
          this.user = res.data;
          this.user.avatar = `https://elm.cangdu.org/img/${this.user.avatar}`;
        });
    },
    urlimg(e) {
      //上传头像
      ...
      this.axios
        .post(
          `http://elm.cangdu.org/eus/v1/users/${localStorage.userid}/avatar`,
          d,
          {
            headers: {
