<template>
	<div id="home">
		...
		<div class="h_ding">
			...
			<div class="h_dq">
				<router-link :to="{ name: 'city', params: { cityid: ding.id ,cityname:ding.name}}" style="display: block;">
		...
		<div>
			<ul class="h_pai">
				<li v-for="(i,key) in cheng" :key="i.cheng" style="margin-bottom: 0.3rem;">
					<h4 style="width:100%;color: #666;font-weight: 400;text-indent: 0.45rem;border-top: 2px solid #e4e4e4;
  border-bottom: 1px solid #e4e4e4;
  font: 0.4rem/1.15rem Helvetica Neue;">{{key}}</h4>
					<ul style="overflow: hidden;">
						<li v-for="a in i" :key="a.no" class="h_lif">
							<router-link :to="{ name: 'city', params: { cityid: a.id ,cityname:a.name}}">{{a.name}}</router-link>
						</li>
					</ul>
				</li>
			</ul>
