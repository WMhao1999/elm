<template>
  <div id="search">
    ...
    <div class="sp_smain" v-if="type">
      ...
      <div>
        <div v-for="i in shopList" :key="i.a">
    ...
    <div class="stroy" v-if="!type">
      <p>历史记录</p>
      <div class="main">
        <div v-for="i in $store.state.StroyShopArr" @click="stroyClick(i)" :key="i.b">{{i}}</div>
      </div>
      <div class="remove" @click="deall">清空所有历史记录</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    upsearch() {
      ...
      this.axios
        .get(
          `https://elm.cangdu.org/v4/restaurants?geohash=${this.$store.state.City.geohash}&keyword=${this.msg}`
        )
        .then(res => {
